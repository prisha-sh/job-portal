...
const RecentJobs = () => {
  ...
  const maxIndex = recentJobs.length - cardsPerView;
  ...
  // Visible slice
  const visibleJobs = recentJobs.slice(currentIndex, currentIndex + cardsPerView);

  return (
    <section className="mt-20 py-16 px-6 relative" style={{ background: '#f1f3f5' }}>
      <style>{dotKeyframes}</style>
      <h2 className="text-3xl font-bold text-gray-800 mb-8 text-center">Recent Jobs</h2>
      {loading ? (
        <div style={pulseAnimationStyle} aria-label="Loading recent jobs" role="status">
